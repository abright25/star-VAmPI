...
def get_all_users():
    try:
        ...
        return Response(error_message_helper("An error occurred while fetching users."), 500, mimetype="application/json")
        ...
        try:
            ...
            if vuln and 'admin' in request_data:  # User is possible to define if she/he wants to be an admin !!
                if request_data['admin']:
                    ...
                    admin = False
                user = User(username=sanitized_username, password=request_data['password'],
...
def update_email(username):
    ...
    if "error" in resp:
        ...
        if vuln:  # Regex DoS
            match = re.search(
                r"^([0-9a-zA-Z]([-.\w]*[0-9a-zA-Z])*@{1}([0-9a-zA-Z][-\